...
for i, line in enumerate(lines):
    if 'path == "/format-paper":' in line:
        lines[i+1] = '        return generate_formatted_paper(payload.get("title", ""), payload.get("text", ""), payload.get("template_name", "Generic University"))\n'
        break
...
if start_index != -1:
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.writelines(lines)
    print("Successfully updated api.py for template-aware styling")
else:
